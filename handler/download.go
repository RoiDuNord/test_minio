...
// 	// totalParts := (stat.Size + chunkSize - 1) / chunkSize
...
// // 	// Используем bufio.Reader с большим буфером
// // 	bufferedReader := bufio.NewReaderSize(content, 1024*1024)
// // 	buf := make([]byte, 1024*1024) // Буфер 1 МБ
...
// // 		}
// // 		if err != nil {
// // 			slog.Error("Ошибка при чтении данных", "fileName", fileName, "error", err)
// // 			return fmt.Errorf("failed to read file data: %v", err)
// // 		}
// // 		if n > 0 {
// // 			_, err := pw.Write(buf[:n])
// // 			if err != nil {
// // 				slog.Error("Ошибка при записи данных клиенту", "fileName", fileName, "error", err)
// // 				return fmt.Errorf("failed to write file data to client: %v", err)
// // 			}
// // 			totalBytes += int64(n)
// // 		}
// // 	}

// // 	slog.Info("Данные переданы клиенту", "fileName", fileName, "duration", time.Since(start).Seconds(), "total_bytes", totalBytes)
// // 	return nil
// // }
...
// // 	fileSize := searchedFile.FileInfo().Size()
// // 	totalChunks := getChunksQuantity(fileSize) // как это передать дальше для асинхронной выгрузки в handlefile
...
// // 	fileSize := searchedFile.FileInfo().Size()
// // 	totalChunks := getChunksQuantity(fileSize)
// // 	slog.Info("Количество чанков для файла", "file", searchedFile.Name, "totalChunks", totalChunks)
...
// // 	if err := handler(w, searchedFile.Name, rc, contentType); err != nil {
// // 		slog.Error("Ошибка при обработке файла из ZIP", "file", searchedFile.Name, "error", err)
// // 		http.Error(w, "Failed to send file data", http.StatusInternalServerError) // here
// // 		return err
// // 	}
...
// // func (s *Server) getObject(ctx context.Context, objectID string, partNumber int) (*minio.Object, error) {
// // return s.MinioClient.Client.GetObject(ctx, s.MinioClient.BucketName, objectID, minio.GetObjectOptions{PartNumber: partNumber})
// // }
...
// // 	// totalChunks := getChunksQuantity(stat.Size)

// // 	// object, err := s.getObject(s.Ctx, objectID, totalChunks)
...
// 	path := "/Users/sitn/Go_Projects/test_minio/handler"
...
// 	maxConcurrent := int64(16) // Можно настроить в зависимости от ваших требований
