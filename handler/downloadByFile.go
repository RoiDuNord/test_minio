...
func (s *Server) Download(w http.ResponseWriter, r *http.Request) {
	...
	if stat.ContentType == "application/zip" {
		...
		s.handleZipFile(w, r, objectID, crc, stat)
		return
	}
	s.handleRegularFile(w, r, objectID, stat)
}

type FileHandler func(w http.ResponseWriter, fileName string, content io.Reader, contentType string, size int) error
...
func parseObjectIDandCRC(r *http.Request) (objectID string, crc32 uint32, err error) {
	...
	if objectID == "" {
		...
		slog.Error("Пустой идентификатор", "error", err)
		return "", 0, err
	}
...
func (pw *ProgressWriter) Write(p []byte) (int, error) {
	...
	pw.totalBytes += n
	now := time.Now()
	if time.Since(pw.last) >= time.Second {
		slog.Info("Прогресс записи", "total_Mb", pw.totalBytes/1024/1024)
		pw.last = now
	}
	return n, nil
}

func handleFile(w http.ResponseWriter, fileName string, content io.Reader, contentType string, fileSize int) error {
	slog.Info("Начало установки заголовков", "fileName", fileName)
	w.Header().Set("Content-Type", contentType)
	w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="%s"`, fileName))
	slog.Info("Заголовки установлены 'Content-Type' и 'Content-Disposition'")

	slog.Info("Начало передачи данных клиенту", "fileName", fileName)
	var writer io.Writer = w
	if fileSize > 10*1024*1024 {
		pw := &ProgressWriter{w: w, last: time.Now(), fileSize: fileSize}
		writer = pw
	}
...
func getChunksQuantity(fileSize int64) int {
	...
	slog.Info("информация о чанках", "fileSize", fileSize, "totalParts", totalParts)
...
func (s *Server) mergeAndSendParts(w http.ResponseWriter, fileName string, contentType string, totalParts int, objectID string) error {
	...
	for i := 1; i <= totalParts; i++ {
		...
		if err != nil {
			...
			return fmt.Errorf("Ошибка записи части файла %d: %v", i, err)
		}
		...
		if err := os.Remove(partPath); err != nil {
			slog.Warn("Не удалось удалить временный файл", "part", i, "path", partPath, "error", err)
		}
	}

	slog.Info("Все части файла успешно объединены и отправлены клиенту", "file_name", fileName, "total_bytes", pw.totalBytes)
	return nil
}

func (s *Server) handleZipFile(w http.ResponseWriter, r *http.Request, objectID string, crc32 uint32, stat minio.ObjectInfo) {
	...
	err = processZip(w, r, object, stat.Size, crc32)
	if err != nil {
	...
	downloadDuration := time.Since(downloadTime).Seconds()
	slog.Info("ZIP-архив обработан и отправлен клиенту", "object_id", objectID, "crc32", crc32, "download_time", downloadDuration)
}

// через файл
func processZip(w http.ResponseWriter, r *http.Request, object *minio.Object, size int64, crc32 uint32) error {
	slog.Info("Начало обработки ZIP-архива", "size_Mb", size/1024/1024)
	...
	searchedFile, err := findSuitableFile(&zipReader.Reader, crc32)
	if err != nil {
		slog.Error("Не удалось найти подходящий файл в ZIP", "error", err, "crc32", crc32)
		return err
	}
	slog.Info("Найден подходящий файл в ZIP-архиве", "file_name", searchedFile.Name, "crc32", crc32)
	...
	if err := handler(w, searchedFile.Name, rc, contentType, int(searchedFile.FileInfo().Size())); err != nil {
...
func (s *Server) handleRegularFile(w http.ResponseWriter, r *http.Request, objectID string, stat minio.ObjectInfo) {
	...
	slog.Info("Количество частей для скачивания", "object_id", objectID, "total_parts", totalParts)
	...
	if totalParts == 1 {
		...
		handler := FileHandler(handleFile)
		if err := handler(w, fileName, object, stat.ContentType, int(stat.Size)); err != nil {
		...
		slog.Info("Файл отправлен клиенту (без разделения на части)", "object_id", objectID, "file_name", fileName, "size", stat.Size, "download_time", downloadDuration)
		return
	}
	...
	downloadDuration := time.Since(downloadTime)
	slog.Info("Файл отправлен клиенту", "object_id", objectID, "file_name", fileName, "size", stat.Size, "download_time", downloadDuration)
}
